...
export ARCH=arm
export UBOOT_MACHINE=vexpress_ca9x4
export KERNEL_MACHINE=vexpress
export TARGET=arm-linux
export GDB_VERSION=8.0
export UBOOT_VERSION=2018.05
...
function download_uboot() {
	if [ -e u-boot-${UBOOT_VERSION}.tar.bz2 ]
	then
		echo u-boot-${UBOOT_VERSION}.tar.bz2 exist
	else
		wget ftp://ftp.denx.de/pub/u-boot/u-boot-${UBOOT_VERSION}.tar.bz2
	fi

	if [ -e u-boot-${UBOOT_VERSION} ]
	then
		echo u-boot-${UBOOT_VERSION}.tar.bz2 exist
	else
		tar jxvf u-boot-${UBOOT_VERSION}.tar.bz2
	fi
}
...
function build_linux() {
	...
	make ${KERNEL_MACHINE}_defconfig
	make zImage -j8
	make modules -j8
	make dtbs
	cd -
